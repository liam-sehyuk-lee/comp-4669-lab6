...
export default {
  ...
  methods: {
    ...
    getPosts() {
      // TODO: get posts from API and set this.posts to the result.
      // The API endpoint is GET /api/posts and it returns a JSON object with a "posts" key that contains an array of posts.
    },
    createPost() {
      // TODO: create a new post using the API and add it to this.posts. The API endpoint is POST /api/posts and it expects a JSON object with a "body" key that contains the post body. 
      // It returns a JSON object with a "post" key that contains the created post.
    },
    deletePost(id) {
      // TODO: delete the post with the given ID using the API and remove it from this.posts. The API endpoint is DELETE /api/posts/{id} where {id} is the ID of the post to delete.
    },
    editPost(post) {
      // TODO: edit the post using the API and update it in this.posts. The API endpoint is PUT /api/posts/{id} where {id} is the ID of the post to edit. 
      // It expects a JSON object with a "body" key that contains the new post body. 
      // It returns a JSON object with a "post" key that contains the updated post.
    }
  },
};
</script>
