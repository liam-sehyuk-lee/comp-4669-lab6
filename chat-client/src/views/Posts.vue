    ...
    <div class="w-4/12">
      <div class="flex flex-row justify-center items-center mt-10">
        <textarea
          name="post_body"
          v-model="body"
          class="textarea textarea-secondary textarea-lg w-full"
          placeholder="What's on your mind?"
        ></textarea>
      </div>
      <div class="flex flex-row">
        <button class="btn btn-primary mt-5 w-full" @click="createPost">
          Post
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    createPost() {
      this.axios.post("/api/posts", { body: this.body }).then((response) => {
        this.posts.unshift(response.data.post);
        this.body = "";
      });
    },
    deletePost(id) {
      this.axios.delete(`/api/posts/${id}`).then(() => {
        this.posts = this.posts.filter((post) => post.id !== id);
      });
    },
    editPost(post) {
      this.axios
        .put(`/api/posts/${post.id}`, { body: post.body })
        .then((response) => {
